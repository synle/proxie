...
1. Set listen address + port (defaults to `127.0.0.1:39871`) and **Save Configuration**.
...
3. Point your system / browser at the proxy. **Both HTTP and HTTPS proxies must be enabled** — HTTPS-only sites won't appear if you toggle only the HTTP proxy.
   - macOS: System Settings → Network → interface → Details → Proxies → enable **Web proxy (HTTP)** AND **Secure web proxy (HTTPS)**, both `127.0.0.1:39871`, click **OK**.
   - Windows: Settings → Network & Internet → Proxy → Manual proxy setup → enable **Use a proxy server**, set address `127.0.0.1` port `39871` (one setting covers both protocols), click **Save**.
   - Linux (GNOME): Settings → Network → Network Proxy → Manual → set HTTP and HTTPS to `127.0.0.1:39871`.
   - Shell: `export HTTP_PROXY=http://127.0.0.1:39871 HTTPS_PROXY=http://127.0.0.1:39871`.
